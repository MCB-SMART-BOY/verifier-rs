...
pub fn check_ctx_access(
    ...
    if !rules.allow_unaligned && !access_off.is_multiple_of(size) {
        return Err(VerifierError::InvalidContextAccess(
            format!("unaligned context access at offset {}", access_off)
        ));
    }
    ...
    let field = rules.find_field(access_off, size)
        .ok_or_else(|| VerifierError::InvalidContextAccess(
            format!("no field at offset {} size {}", access_off, size)
        ))?;
    ...
    if is_write && !field.writable {
        return Err(VerifierError::InvalidContextAccess(
            format!("field at offset {} is not writable", access_off)
        ));
    }
    if !is_write && !field.readable {
        return Err(VerifierError::InvalidContextAccess(
            format!("field at offset {} is not readable", access_off)
        ));
    }
...
pub fn convert_ctx_to_packet_access(
...
pub fn get_skb_rewrite(offset: u32, size: u32) -> Option<CtxRewrite> {
    ...
    match offset {
        0 => Some(CtxRewrite { orig_offset: 0, new_offset: 104, size, needs_helper: false }), // len
        76 => Some(CtxRewrite { orig_offset: 76, new_offset: 200, size, needs_helper: false }), // data
        80 => Some(CtxRewrite { orig_offset: 80, new_offset: 208, size, needs_helper: false }), // data_end
        _ => None,
    }
}
