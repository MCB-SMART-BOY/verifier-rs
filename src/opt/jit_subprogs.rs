...
pub fn jit_subprogs(env: &mut VerifierEnv, insns: &mut [BpfInsn]) -> Result<JitSubprogContext> {
...
fn patch_subprog_calls(ctx: &mut JitSubprogContext, insns: &mut [BpfInsn]) -> Result<()> {
...
pub fn fixup_subprog_calls_after_patch(insns: &mut [BpfInsn], patch_idx: usize, delta: i32) {
    for (i, insn) in insns.iter_mut().enumerate() {
        if insn.code == (BPF_JMP | BPF_CALL) && insn.src_reg == BPF_PSEUDO_CALL {
...
pub fn extract_subprog(insns: &[BpfInsn], sp: &SubprogInfo) -> Vec<BpfInsn> {
    insns[sp.start..sp.end].to_vec()
}

/// Verify that all subprogram calls have valid targets
pub fn verify_subprog_calls(insns: &[BpfInsn], ctx: &JitSubprogContext) -> Result<()> {
...
pub fn propagate_subprog_properties(ctx: &mut JitSubprogContext, insns: &[BpfInsn]) -> Result<()> {
    let n = ctx.count();
    if n == 0 {
        return Ok(());
    }
...
pub fn determine_priv_stack_usage(ctx: &mut JitSubprogContext, insns: &[BpfInsn]) -> Result<()> {
...
fn calculate_call_depths(ctx: &JitSubprogContext, insns: &[BpfInsn]) -> Result<Vec<u32>> {
    let n = ctx.count();
...
pub fn validate_combined_stack_depth(ctx: &JitSubprogContext, insns: &[BpfInsn]) -> Result<i32> {
    let depths = calculate_call_depths(ctx, insns)?;
...
pub fn max_call_depth(insns: &[BpfInsn], ctx: &JitSubprogContext) -> Result<u32> {
            ...
            for idx in sp.start..sp.end {
                if let Some(insn) = insns.get(idx) {
                    if insn.code == (BPF_JMP | BPF_CALL) && insn.src_reg == BPF_PSEUDO_CALL {
                        ...
                        if let Some(target_sp) = ctx.find_subprog(target) {
                            let sub_depth =
                                dfs(target_sp, insns, ctx, visited, depth, current_depth + 1)?;
