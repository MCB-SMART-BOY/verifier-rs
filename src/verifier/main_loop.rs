...
use crate::analysis::prune::{
    clean_live_states, is_state_visited, JmpHistory, SccTracker, StateCache, StateId,
    StateVisitContext, StateVisitResult,
};
use crate::check::callback::CallbackState;
use crate::check::jump::{
    check_indirect_jump, check_indirect_jump_fields, check_may_goto, is_indirect_jump,
    is_may_goto_insn, validate_indirect_jump_targets, MayGotoState,
};
...
use crate::special::exception::{
    is_bpf_throw_kfunc, setup_exception_callback_state, validate_bpf_throw, ExceptionState,
};
...
impl<'a> MainVerifier<'a> {
        ...
        loop {
            ...
            match result {
                ...
                InsnResult::Branch(fall_through, target) => {
                    ...
                    let insn = *self
                        .env
                        .insn(self.env.insn_idx)
                        .ok_or(VerifierError::InvalidInsnIdx(self.env.insn_idx))?;
        ...
        for (i, subprog) in self.env.subprogs.iter().enumerate() {
            if subprog.start >= self.env.prog_len() {
                return Err(VerifierError::InvalidSubprog(format!(
                    "subprog {} start {} out of range",
                    i, subprog.start
                )));
            }
            if subprog.end > self.env.prog_len() {
                return Err(VerifierError::InvalidSubprog(format!(
                    "subprog {} end {} out of range",
                    i, subprog.end
                )));
            }
            if subprog.start >= subprog.end {
                return Err(VerifierError::InvalidSubprog(format!(
                    "subprog {} has invalid range [{}, {})",
    ...
    fn check_state_visited(&mut self) -> Result<Option<StateVisitResult>> {
        ...
        match &result {
            StateVisitResult::Explore(new_state_id) => {
                ...
                self.scc_tracker
                    .maybe_enter_scc(cur_state, *new_state_id, &self.env.insn_aux);
            }
            ...
            StateVisitResult::Prune(matched_id) => {
                ...
                if is_loop {
                    ...
                    let _ =
                        self.scc_tracker
                            .add_backedge(cur_state, *matched_id, &self.env.insn_aux);
                }
    ...
    fn check_ld(&mut self, insn: &BpfInsn, idx: usize) -> Result<InsnResult> {
        ...
        match mode {
            ...
            _ => Err(VerifierError::InvalidInstruction(idx)),
        }
    }
    ...
    fn process_bpf_exit(&mut self, exception_exit: bool) -> Result<InsnResult> {
        let state = self
            ...
            .cur_state
            .as_ref()
            .ok_or(VerifierError::Internal("no state at exit".into()))?;
    ...
    fn check_retval_in_range(&self, r0: &BpfRegState, min_val: i64, max_val: i64) -> Result<()> {
        ...
        if r0_min > max_val || r0_max < min_val {
            return Err(VerifierError::InvalidReturnValue(format!(
                "return value range [{}, {}] outside valid range [{}, {}]",
        ...
        if r0_min < min_val || r0_max > max_val {
            ...
            if !self.env.caps.bounded_loops {
                ...
                return Err(VerifierError::InvalidReturnValue(format!(
                    "return value range [{}, {}] may exceed valid range [{}, {}]",
                    r0_min, r0_max, min_val, max_val
                )));
            }
        }
    ...
    fn check_async_callback_return(
        &self,
        r0: &BpfRegState,
    ...
    fn check_subprog_return(&self, r0: &BpfRegState) -> Result<()> {
        ...
        if r0.reg_type != BpfRegType::ScalarValue && r0.reg_type != BpfRegType::NotInit {
            ...
            if !r0.is_pointer() || r0.ref_obj_id == 0 {
                return Err(VerifierError::InvalidReturnValue(format!(
                    "subprogram must return scalar, got {:?}",
                    r0.reg_type
                )));
            }
        }
    ...
    fn check_cond_jmp(&mut self, insn: &BpfInsn, idx: usize) -> Result<InsnResult> {
        ...
        match (path1, path2) {
            ...
            _ => Err(VerifierError::Internal("invalid branch result".into())),
        }
    }
    ...
    fn split_branch_states(
        ...
        use crate::bounds::range_refine::{refine_reg_const, refine_regs, BranchCond};
        ...
        let src_val = if src_type == BPF_X {
            ...
            let cur_state = self
                ...
                .cur_state
                .as_ref()
                .ok_or_else(|| VerifierError::Internal("no current state".into()))?;
    ...
    fn refine_ptr_null_check_static(
        ...
        let dst_maybe_null = false_state
            .reg(dst_reg)
    ...
    fn mark_ptr_or_null_in_state_static(state: &mut BpfVerifierState, id: u32, is_null: bool) {
        if id == 0 {
            return;
        }
            ...
            if let Some(Some(func)) = state.frame.get_mut(frame_idx) {
                ...
                for slot in &mut func.stack.stack {
                    if slot.slot_type[BPF_REG_SIZE - 1] == BpfStackSlotType::Spill {
                        ...
                        if spilled.id == id
                            && spilled.type_flags.contains(BpfTypeFlag::PTR_MAYBE_NULL)
                        {
                            if is_null {
    ...
    fn check_helper_call(&mut self, insn: &BpfInsn, idx: usize) -> Result<()> {
        ...
        check_helper_call_with_ctx(state, id_gen, func_id, idx, allow_ptr_leaks, &user_ctx)?;
    ...
    fn check_kfunc_call(&mut self, insn: &BpfInsn) -> Result<InsnResult> {
        ...
        if let Some(desc) = desc {
            ...
            let meta = check_kfunc_call(state, &self.kfunc_registry, insn, insn_idx)?;
    ...
    fn handle_bpf_throw(&mut self, _insn: &BpfInsn) -> Result<InsnResult> {
        ...
        if let Some(callback_subprog) = self.env.exception_callback_subprog {
            ...
            if let Some(callback) = self.exception_state.callbacks.values().next() {
                ...
                let callback_insn = self
                    .env
                    .subprogs
                    .get(callback_subprog)
                    .map(|s| s.start)
                    .unwrap_or(0);
