...
impl SccVisitInfo {
    ...
    pub fn get_or_create_visit(&mut self, callchain: SccCallchain) -> &mut SccVisit {
        if self.find_visit(&callchain).is_none() {
            self.visits.push(SccVisit::new(callchain.clone()));
        }
        self.find_visit_mut(&callchain).unwrap()
    }
}
...
impl SccTracker {
    ...
    pub fn maybe_enter_scc(
        ...
        let visit = info.get_or_create_visit(callchain);
